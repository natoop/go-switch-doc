...
-   **完整 TUI 界面**：运行 `cc-switch` 后进入终端图形界面，可以像使用桌面版一样新增 Provider、选择 GoSwitcher 模板、填写 API Key、保存并切换配置。

如果你只是第一次配置 GoSwitcher，推荐先用 TUI。配置完成后，日常切换、检查和排错可以直接用 CLI 命令完成。
...
TUI 适合第一次配置。你可以在里面选择 GoSwitcher 模板，填入 API Key，然后保存并切换到该 Provider。
...
第一次配置推荐使用 TUI，因为它会展示 GoSwitcher 模板和需要填写的字段。
...
``` text
帮我运行 cc-switch --app codex provider current，确认 Codex 当前是不是 GoSwitcher。
```
...
``` text
帮我切换到 GoSwitcher provider，然后运行 claude 测试是否能正常回复。
```
