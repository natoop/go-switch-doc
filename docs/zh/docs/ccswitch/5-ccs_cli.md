...
-   **完整 TUI 界面**：运行 `cc-switch` 后进入终端图形界面，可以像使用桌面版一样新增 Provider、选择 GoSwitch 模板、填写 API Key、保存并切换配置。

如果你只是第一次配置 GoSwitch，推荐先用 TUI。配置完成后，日常切换、检查和排错可以直接用 CLI 命令完成。
...
TUI 适合第一次配置。你可以在里面选择 GoSwitch 模板，填入 API Key，然后保存并切换到该 Provider。
...
第一次配置推荐使用 TUI，因为它会展示 GoSwitch 模板和需要填写的字段。
...
4.  在 `API Key` 中填入你从 GoSwitch 复制的令牌，然后保存。

![](../../../assets/image/CC-Switch/021.webp)

5.  回到供应商列表，确认当前选中的是刚刚添加的 GoSwitch Provider。
...
``` text
帮我运行 cc-switch --app codex provider current，确认 Codex 当前是不是 GoSwitch。
```
...
``` text
帮我切换到 GoSwitch provider，然后运行 claude 测试是否能正常回复。
```
